window.addEventListener("load", (event) => {
  ...
  const validationFixButton = document.getElementsByClassName("validation-fix-button");
  ...
  for (let i = 0; i < validationFixButton.length; i++) {
    validationFixButton[i].addEventListener("click", function (e) {
      ...
      switch(subject) {
        case "Milestone":
          e.target.innerHTML = "Adding milestones <i class='fa fa-fw fa-spinner fa-spin'></i>";
          async function fetchData() {
            let response = await fetch('/EstimateImport/importValidation?fixErrors='+subject);
